...
const brokerConfig: BrokerOptions = {
  ...
  created(broker: ServiceBroker) {
    broker.logger.info(
      `ServiceBroker: ${broker.namespace}:${broker.nodeID} created.`
    );
  },
  // Called after broker started.
  started(broker: ServiceBroker) {
    broker.logger.info(
      `ServiceBroker: ${broker.namespace}:${broker.nodeID} started.`
    );
  },
  // Called after broker stopped.
  stopped(broker: ServiceBroker) {
    broker.logger.info(
      `ServiceBroker: ${broker.namespace}:${broker.nodeID} stopped.`
    );
  }
};
