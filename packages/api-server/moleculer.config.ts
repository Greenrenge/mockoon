...
const brokerConfig: BrokerOptions = {
	...
	//   requestTimeout:
	//     typeof requestTimeout !== 'number' ? 10 * 1000 : requestTimeout,
	...
	// errorHandler: null,
	// @ts-ignore there is no error handler in the broker options
	errorHandler(
		this: ServiceBroker,
		err: any,
		{
	...
	created(broker: ServiceBroker) {
		broker.logger.info(`ServiceBroker: ${broker.namespace}:${broker.nodeID} created.`)
	},
	// Called after broker started.
	started(broker: ServiceBroker) {
		broker.logger.info(`ServiceBroker: ${broker.namespace}:${broker.nodeID} started.`)
	},
	// Called after broker stopped.
	stopped(broker: ServiceBroker) {
		broker.logger.info(`ServiceBroker: ${broker.namespace}:${broker.nodeID} stopped.`)
	},
}
